...
4. **Make the art** - follow `references/art-spec.md` (35 px chibi, 1 px black outline, no
   anti-aliasing, hard 3-4 step shading, facing right; saturated outline-free VFX). Hand-drawn
   or generated by an image model and imported with `scripts/strips.py` (art-spec "Generated
   art"). Check with `python scripts/tfm2_ase.py metrics <sprite>` and preview with `render`.
