...
生成的 PNG 放进一个文件夹交给 Claude。Claude 会按 8×8 的方块逐格取色，放回现在每一帧的位置，接回游戏（`tools/art/import_native.py`，导入时再写）。
...
| | 原版英雄 | 盖伦 | 艾希 / 拉克丝 |
|---|---|---|---|
| 颜色数（待机第 1 帧） | 16–28 | 63 | 42 / 41 |
| 和右边像素同色的比例 | 34%–46% | 15% | 15% / 18% |
...
- 帧数、动作名、帧时长和现在一样（`tools/art/import_lux.py`、`tools/art/import_ashe.py` 里的 `CHAR`）。
- 每张图先量出方块大小和网格（约 8 px；GPT 的输出尺寸可能和要求不同），逐格取方块中心的颜色，还原成游戏像素。
- 按格子切出每帧，和现在同一帧的轮廓做重合度匹配（上下左右最多挪几个像素），放回现在这一帧的位置。这样头部轨迹、前冲距离、R 的法杖高度、死亡击飞都不变。
- 调色板每个英雄不超过 24 色；轮廓缺描边的地方补 1 px 近黑色。
- 验收：
  - 颜色数和"与右邻像素同色的比例"要接近原版英雄（16–28 色、34%–46%）；
  - `tfm2_ase.py face` 看得到眼睛；
  - `lint_mod.py` 0 错误。

参考图用 `python tools/art/native_refs.py --out <文件夹> --hero lux --hero ashe --style` 重新生成（原版英雄对照图从游戏的 `bundle.game_data` 读取）。
